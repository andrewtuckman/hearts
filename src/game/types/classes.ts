...
export class Game {
    ...
    removeCardFromHand(pid: PID, card: Card) : Card {
        ...
        if (cardIndex === -1) {
            throw new Error(`Card ${card.rank} of ${card.suit} not found in player ${pid}'s hand.`);
        }
        const removedCards = hand.splice(cardIndex, 1);
        if (removedCards.length !== 1) {
            throw new Error(`Unexpected error removing card ${card.rank} of ${card.suit} from player ${pid}'s hand.`);
        }
        if (removedCards[0].suit !== card.suit || removedCards[0].rank !== card.rank) {
            throw new Error(`Removed card does not match the requested card for player ${pid}.`);
        }
